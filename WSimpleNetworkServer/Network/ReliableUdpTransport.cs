...
public class ReliableUdpTransport : ITransport
{
    ...
    // 配置参数
    private const int RetransmitTimeoutMs = 1000;   //重传超时界限
    private const int CleanupTimeoutMs = 30000;     //会话超时界限
    ...
    public ReliableUdpTransport(string ip, int port)
    {
        ...
        _isServer = false;
        _retransmitTimer = new Timer(CheckRetransmit, null, 100, 100);
        _cleanupTimer = new Timer(CleanupSessions, null, 100, 100);
    ...
    private void HandleDataPacket(Packet packet, IPEndPoint senderEndPoint)
    {
        ...
        Console.WriteLine($"[Transport] 收到数据包从{senderEndPoint} SeqNum={packet.SequenceNumber}，DataLen={packet.Data.Length}");
        
        var ackPacket = Packet.CreateAckPacket(packet.SequenceNumber);
        SendPacketTo(ackPacket, senderEndPoint);
        Console.WriteLine($"[Transport] 发送 ACK 到 {senderEndPoint} SeqNum={packet.SequenceNumber}");
